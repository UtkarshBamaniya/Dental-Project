...
const openEdit = async (id) => {
    ...
    try {
        const response = await axios.get(route('medical-details.edit', id), {
            headers: {
...
const submit = () => {
    const action = isEditMode.value
        ? form.put(route('medical-details.update', form.id), {
            preserveScroll: true,
            onSuccess: () => {
                hideDialog();
                emit('fetch-data');
        ...
        : form.post(route('medical-details.store'), {
            preserveScroll: true,
            onSuccess: () => {
                hideDialog();
                emit('fetch-data');
...
<template>
    <Dialog
        ...
        modal
        :header="isEditMode ? 'Edit Medical Detail' : 'Add Medical Detail'"
        :style="{ width: 'min(720px, 96vw)' }"
    >
        <div v-if="loading" class="py-10 text-center text-sm text-slate-500">Loading medical detail...</div>
